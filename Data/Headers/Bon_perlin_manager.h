...
int make_rnd_number_4gen(int min, int max){
 std::random_device _rrd;
 std::mt19937 mt(_rrd());
 std::uniform_real_distribution<double> _rdist(min, max);
 float _rnd = _rdist(mt);
 unsigned int numrnd_gen0xxfol = _rnd;
 return numrnd_gen0xxfol;
}
/*
