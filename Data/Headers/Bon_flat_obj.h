class Flat_v{
 ...
 public:
  ...
  int lenc = 0;
  int isCube = 0;
  void Flat_ini(float _scala, std::string _url, std::string _nme, int _oid, std::string _cod, int _ic);
  void Update_code();
  void Start_code_main();
  void DrawFlatLookAt(int _map[16][255][16],int x, int y, int z,float lx,float ly,float lz,int _x, int _y, int _z);
  void runln(std::string script_cmd);
};

void Flat_v::Flat_ini(float _scala, std::string _url, std::string _nme, int _oid, std::string _cod, int _ic){
 scala = _scala;
 bs_id = _oid;
 bool is_func = false;
 oname = _nme;
 ...
 if(_ic==1){
  tex_cust = Bon_load_bmp(lnk, 64, 192);           // load the player texture as .bmp into the game
 }else{
...
void Flat_v::DrawFlatLookAt(int _map[16][255][16], int x, int y, int z,float lx,float ly,float lz,int _x, int _y, int _z){
 if(isCube==0){
 ...
 glPopMatrix();
  ...
  glPushMatrix();                              // move this block individually
   glBindTexture(GL_TEXTURE_2D, tex_cust); // set stone texture to block
   Bon_draw_cube(_map,_x,_y,_z,x,y,z,0.5); // calculate and draw optimized cube with it's position
  glPopMatrix();
 }
}
