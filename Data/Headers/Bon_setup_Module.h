...
void Initialize_custom(){
 ...
 if(res_set!=NULL){ 
  ...
  int o_ptr = 0;
  int ot_id=0; std::string ot_code, ot_name, ot_url; int is_Cube = 0;
  while((row=mysql_fetch_row(res_set))!=NULL){
   ...
   is_Cube = atoi(row[5]);
   ot_name = row[1];
   object_array[o_ptr].Flat_ini(0.5,ot_url,ot_name,array_limit+ot_id-1,ot_code,is_Cube);
   o_ptr++;
  }
 }
 mysql_free_result(res_set);
