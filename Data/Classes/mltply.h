...
class Player_manager{
 public: 
  ...
  void Player_Joined( int _id, std::string _usr, std::string _url , int _pushed,float _x,float _y, float _z, int _r, int _v);
...
void Player_manager::Player_Initialize(){                                  
 ...
 std::string tul = row[8];                      // convert URL of skin into string
  ...
  Player_Joined(atoi(row[0]),tus,tul, p_ptr,atof(row[3]),atof(row[4]),atof(row[5]),atoi(row[6]),atoi(row[10]));
...
void Player_manager::Player_Update(){
 ...
 float txp = 0, typ = 0, tzp = 0; int trt = 0, tid = 0,tptr = 1,itmp = 0; int tvs; // some temporal variables
 ...
 while(players_i[itmp] >= 0 && itmp < 100){  // while the player is inside the ranking of nodes that can be supported by the game
  ...
  //std::cout<<"update players download"<<std::endl;  // show an important debug message
   ...
   int visb = players_visible[itmp];
   Player_Joined(players_i[itmp],tus,tul, p_ptr,players_x[itmp],players_y[itmp],players_z[itmp],players_r[itmp],visb);
...
########################################################################################
*/
void Player_manager::Player_Joined( int _id, std::string _usr, std::string _url , int _pushed,float _x,float _y, float _z, int _r, int _v){
...
void Player_manager::Player_Drawall(){
 ...
 for(int i = 0; i < p_ptr; i++){        // loop through all players in game
  ...
  //std::cout<<"testing for draw"<<std::endl;
   ...
   //std::cout<<"player visibility: "<<auxiliar->visible<<std::endl;
   if(auxiliar->visible > 0){ // if it's visible
  ...
  if(isRunning == true){                     // If update is enabled
    ...
    */
    ...
    while((row = mysql_fetch_row(res_set))!=NULL){    // while im receiving information 
      ...
      players_visible[data_number_p] = atoi(row[10]);  // 
    ...
    std::string sysreq = "SELECT * FROM System_request ORDER BY Func_ID DESC LIMIT 6;";
    
    mysql_query(connect, sysreq.c_str());          // query to receive all player messages
    
    res_set = mysql_store_result(connect);         // store messages inside the 
    
    while((row=mysql_fetch_row(res_set))!=NULL){   // while it's fetching messages
     system_event_id[rq_ptr] = atoi(row[0]);
     sysreqs[rq_ptr] = row[1];                     // save messages inside an array
     rq_ptr++;                                     // just for array movement
    }
    rq_ptr = 0;                                    // reset the pointer
    ...
    if((is_texting == false) && (strlen(chat_tosend.c_str())) > 0 ){  // if player is not testing anymore and there is a message into the stack    
     /*
     --------------------------------------
     - TEST IF IT'S A COMMAND OR A CHAT   -
     --------------------------------------
     */
     char temporal[128];  
     
     char trash[128]; // username
     char instr[128]; // test if command
     char comnd[128]; // the command
     
     strcpy(temporal, chat_tosend.c_str()); 
     sscanf(temporal, "%s %s %s", &trash, &instr, &comnd);
     
     if(strcmp(trash, "!execute") == 0){
      msg = "INSERT INTO System_request (FPacket, Post_tm, Dest_jc) VALUES (\'"+chat_tosend+"\', 0, 1);";
     }else if(strcmp(trash, "!music") == 0){
      play_music = true;
     }else{
      msg = "INSERT INTO chat_data (Msg_Data,User_Sent,Session) VALUES ('<"+tmp_username+">> "+chat_tosend+"','"+tmp_username+"',"+std::to_string(current_sesion)+");"; // send to mysql the message
     }
