        ...
        if((aux->chunk_data[x][y][z]) <= array_limit){
         for(int i = 0; i < obj_num; i++){
           if(object_array[i].bs_id == (aux->chunk_data[x][y][z])){
            object_array[i].DrawFlatLookAt(aux->chunk_data,x+(aux->zone_X*15),y,z+(aux->zone_Y*15),px,py,pz,x,y,z);
           }
         }
        }
       break;
...
void Chunk_gen::Chunk_Create( int p, int zx, int zy ){ // function to create new node of chunk with data
 ...
 // ---------------------------------------------------
    // TODO BUILDINGS GO HERE DO NOT CONFUSEE TODO 
    // ---------------------------------------------------
    // ----------------------------------------------------------------------
    // - GENERATION OF ALL THE NON-NATURAL STRUCTURES (WALLS, FLOOR ETC,    -
    // ----------------------------------------------------------------------
      if(wrl_md_void != 1){
      // --------- IF STRUCTURES MUST GENERATE ROOFS THEN ---------------------
       // ---------------------------------------------------------
       // -       GENERATION OF THE WALLS INSIDE THE WORLD        -
       // ---------------------------------------------------------
         int bwall_left = 0;
         int bwall_right= 0;
         int bwall_front= 0;
         ...
         if(wleft_wall_gen_mode!=0){
           if(wleft_wall_gen_mode==1){
            int has_lwall_gen = make_rnd_number_4gen(1, 10);
            if(has_lwall_gen > 8){
              bwall_left = 1;
            }else{
              bwall_left = 0;
         ...
         if(right_wall_gen_mode!=0){
           if(right_wall_gen_mode==1){
            int has_rwall_gen = make_rnd_number_4gen(1, 10);
            if(has_rwall_gen > 8){
              bwall_right = 1;
            }else{
              bwall_right = 0;
         ...
         if(front_wall_gen_mode!=0){
           if(front_wall_gen_mode==1){
            int has_fwall_gen = make_rnd_number_4gen(1, 10);
            if(has_fwall_gen > 8){
              bwall_front = 1;
            }else{
              bwall_front = 0;
         ...
         if(wback_wall_gen_mode!=0){
           if(wback_wall_gen_mode==1){
            int has_bwall_gen = make_rnd_number_4gen(1, 10);
            if(has_bwall_gen > 8){
              bwall_back = 1;
            }else{
              bwall_back = 0;
         ...
         // --------------- ACTUALLY BUILD THE WALL GENERATION ------------------------------
         if(wleft_wall_gen_mode!=0||wback_wall_gen_mode!=0||front_wall_gen_mode!=0||right_wall_gen_mode!=0){
          for(int rfgx = 1; rfgx < 15; rfgx++){
                  for(int rfgy = min_wall_size; rfgy < max_wall_size; rfgy++){
                    for(int rfgz = 1; rfgz < 15; rfgz++){
                       // -------- ROOF MODE APPLY HERE --------------------
                       newchunk->chunk_data[rfgx][rfgy][rfgz] = base_walls;
                       if(rfgx > 1+bwall_left && rfgx < 15-bwall_right &&
                          rfgz > 1+bwall_back && rfgz < 15-bwall_front ){
                         newchunk->chunk_data[rfgx][rfgy][rfgz] = 0;
                       }
                       // --------------------------------------------------
                       newchunk->chunk_data[8][rfgy][1] = 0; newchunk->chunk_data[8][rfgy][15] = 0;
                       newchunk->chunk_data[7][rfgy][1] = 0; newchunk->chunk_data[7][rfgy][15] = 0;
                       newchunk->chunk_data[9][rfgy][1] = 0; newchunk->chunk_data[9][rfgy][15] = 0;
                       
                       newchunk->chunk_data[1][rfgy][8] = 0; newchunk->chunk_data[15][rfgy][8] = 0;
                       newchunk->chunk_data[1][rfgy][7] = 0; newchunk->chunk_data[15][rfgy][7] = 0;
                       newchunk->chunk_data[1][rfgy][9] = 0; newchunk->chunk_data[15][rfgy][9] = 0;
       ...
       if(roof_define_gen_mode!=0){
          // -----------------------------
             if(roof_define_gen_mode==1){
             // ------------------------------------------------------------
                std::random_device rrd;
                std::mt19937 mt(rrd());
                std::uniform_real_distribution<double> rdist(1.0, 10.0);
                float rnd = rdist(mt);
                unsigned int roof_gen = rnd;
                for(int rfgx = 0; rfgx < 16; rfgx++){
                  for(int rfgy = 0; rfgy < 255; rfgy+=roof_frequency_of_gen){
                    for(int rfgz = 0; rfgz < 16; rfgz++){
                       // -------- ROOF MODE APPLY HERE --------------------
                       newchunk->chunk_data[rfgx][rfgy][rfgz] = base_roofs;
                       if(rfgx > 1 && rfgx < 15 && rfgz > 1 && rfgz < 15 && rnd > 5){
                         newchunk->chunk_data[rfgx][rfgy][rfgz] = 0;
                       }
                       // --------------------------------------------------
                    }
 ...
 // ----------------------------------------------------------------------------
 // - TODO: CHUNK WORLD GENERATION IMPROVED FOR THE CREATION OF LIMINAL SPACES -
 // ----------------------------------------------------------------------------
   if(wrl_md_void != 1){  // --- IF GENERATION IS NOT VOID 
    // ---------------------------------------------------
      if(is_flatw==true){ // --- IF IS A FLAT WORLD
      // ------------------------------------------
        for(int z = 0; z < 16; z++){                          // this is the world generation
         for(int y = 0; y < floor_base_size; y++){            // as this depends on modes or anything
          for(int x = 0; x < 16; x++){                        // since it's mode 0 generates a flat terrain
            if(y < floor_base_size-1){                        // inside the chunk which means
              newchunk->chunk_data[x][y][z] = secn_floor;     // this sets information inside the main array
            }else{
              newchunk->chunk_data[x][y][z] = base_floor;     // of the chunk data  
              // ----------------------------------------
              //         GRASS AND TREES ROUTINES        
              // ----------------------------------------
              if(has_grass == true){
               if(has_beach == true && y >= sand_min_gen && y <= sand_max_gen){
                 newchunk->chunk_data[x][y+1][z] = 0;
               }else{
              ...
              if(has_trees==true){
               // --------- TREE LEAVES ----------------------------------------------------
               for(int tgx = 0; tgx < 5; tgx++){                              // the X size of leaves
                 for(int tgy = 0; tgy < 4; tgy++){                            // the Y size of leaves
                  for(int tgz = 0; tgz < 5; tgz++){                           // the Z size of leaves
                   if(newchunk->chunk_data[tgx][tgy][tgz] > 0){               // IF the block is empty
                     newchunk->chunk_data[6+tgx][floor_base_size+tgy+6][6+tgz] = 5; // add a leaves block on space
                   }
                  }                                                     // close the leave generation routine
                 }
                }
                // ---------------------------------------------------------------------------
                for(int f = floor_base_size; f < (floor_base_size+6); f++){
                  if(f != floor_base_size){
            ...
            // -------- MAKE BEACH ----------
              if(y >= sand_min_gen && y <= sand_max_gen && has_beach == true){
                newchunk->chunk_data[x][y][z] = 15;
              }
            // ------------------------------
      ...
      // ------------------------------------------
      else if(is_isle == 1){ // --- ARE ISLES WORLD
      // ---------------------------------------------------------------------------------------------------------------------
        for(int x = 0; x < 16; x++){
         for(int z = 0; z < 16; z++){
          height_map1 = floor_base_size+ValueNoise_2D((float)(std::abs(zx*16+x)), (float)(std::abs(zy*16+z)))*world_freq;
          ...
          // ----------------------------
           ...
           std::mt19937 mt(rd());
           int rnd_min_val_gen = 1;
           int rnd_max_val_gen = floor_base_size;
           if(floor_base_size>15){
            rnd_min_val_gen = floor_base_size-5;
            rnd_max_val_gen = floor_base_size-2;
           }else{
            rnd_min_val_gen = floor_base_size;
            rnd_max_val_gen = floor_base_size+1;
           }
           std::uniform_real_distribution<double> dist(rnd_min_val_gen, rnd_max_val_gen);
           float hhrnd = dist(mt);
           unsigned int rnd_ht = hhrnd;
          // ----------------------------
          for(int y = rnd_ht; y < height_map1; y++){
            if(height_map1 > floor_base_size+5){
              if(y < height_map1-1){                                  
      	        newchunk->chunk_data[x][y][z] = secn_floor;                
     	      }else{
      	        newchunk->chunk_data[x][y][z] = base_floor;    
      	        // ----------------------------------------
                //         GRASS AND TREES ROUTINES        
                // ----------------------------------------
                if(has_grass == true){
                ...
                // ----------------------------------------  
                if(has_trees==true){
                int tgv02 = floor_base_size+ValueNoise_2D((float)(std::abs(zx*16+8)), (float)(std::abs(zy*16+8)))*world_freq;
               // --------- TREE LEAVES ----------------------------------------------------
               for(int tgx = 0; tgx < 5; tgx++){                              // the X size of leaves
                 for(int tgy = 0; tgy < 4; tgy++){                            // the Y size of leaves
                  for(int tgz = 0; tgz < 5; tgz++){                           // the Z size of leaves
                   //if(newchunk->chunk_data[tgx][tgy][tgz] > 0){               // IF the block is empty
                     newchunk->chunk_data[6+tgx][tgv02+tgy+6][6+tgz] = 5; // add a leaves block on space
                   //}
                  }                                                     // close the leave generation routine
                 }
                }
                // ---------------------------------------------------------------------------
                for(int f = tgv02; f < (tgv02+6); f++){
                  if(f != tgv02){
                    newchunk->chunk_data[8][f][8] = 4;                   // this is just a random stick on chunks ignore it
                  }else{
                    newchunk->chunk_data[8][f][8] = 7;                   // this is just a random stick on chunks ignore it
      ...
      // ------------------------------------------
      else if(is_flatw == false && is_isle == 0){ // NORMAL PERLIN
       // ---------------------------------------------------------------------------------------------------------------------
        for(int x = 0; x < 16; x++){
         for(int z = 0; z < 16; z++){
          height_map1 = floor_base_size+ValueNoise_2D((float)(std::abs(zx*16+x)), (float)(std::abs(zy*16+z)))*world_freq;
          ...
          for(int y = 0; y < height_map1; y++){
            if(y < height_map1-1){                                  
              ...
              // ----------------------------------------
              if(has_trees==true){
               int tgv03 = floor_base_size+ValueNoise_2D((float)(std::abs(zx*16+8)), (float)(std::abs(zy*16+8)))*world_freq;
               // --------- TREE LEAVES ----------------------------------------------------
               for(int tgx = 0; tgx < 5; tgx++){                              // the X size of leaves
                 for(int tgy = 0; tgy < 4; tgy++){                            // the Y size of leaves
                  for(int tgz = 0; tgz < 5; tgz++){                           // the Z size of leaves
                   //if(newchunk->chunk_data[tgx][tgy][tgz] > 0){               // IF the block is empty
                     newchunk->chunk_data[6+tgx][tgv03+tgy+6][6+tgz] = 5; // add a leaves block on space
                   //}
                  }                                                     // close the leave generation routine
                 }
                }
                // ---------------------------------------------------------------------------
                for(int f = tgv03; f < (tgv03+6); f++){
                  if(f != tgv03){
                    newchunk->chunk_data[8][f][8] = 4;                   // this is just a random stick on chunks ignore it
                  }else{
                    newchunk->chunk_data[8][f][8] = 7;                   // this is just a random stick on chunks ignore it
