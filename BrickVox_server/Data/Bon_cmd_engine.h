...
bool test_cmd(std::string cmd2rd){
    bsplit(cmd2rd, ' ');
    if(strcmp(splitted[0].c_str(), "die")==0){
        std::string qry = "INSERT INTO System_request (FPacket, Post_tm, Dest_jc) VALUES (\'!shutdown\', 0, 1);";
    ...
    if(strcmp(splitted[0].c_str(), "kick")==0){
     std::string qry = "INSERT INTO System_request (FPacket, Post_tm, Dest_jc) VALUES (\'!kick "+splitted[1]+"\', 0, 1);";
     
     if(mysql_query(connect,qry.c_str())){
       std::cout<<mysql_errno(connect)<<std::endl;
     }
     //------------------------------
     std::string qry2 = "UPDATE Players_ingame SET is_visible = 0 WHERE Username = \'" + splitted[1] + "\';";
     
     if(mysql_query(connect,qry2.c_str())){
       std::cout<<mysql_errno(connect)<<std::endl;
     }
     return true;
    }
    // ---------------- DEFAULT STATUS IN CASE NO COMAND FITS -----------------------------
     std::string qry = "INSERT INTO System_request (FPacket, Post_tm, Dest_jc) VALUES (\'!execute "+cmd2rd+"\', 0, 1);";
